		...
		fields: [
			{
				...
				label: "Role/Title",
				type: "text",
				placeholder: "Expert, Witness, etc.",
			},
			{
				name: "organization",
				label: "Organization",
...
export class SourcePillInput {
	...
	saveEditor() {
		...
		const pill = this.pillGrid.querySelector(
			`[data-index="${this.editingIndex}"]`,
		);
		if (!pill) {
			this.closeEditor();
			return;
	...
	renderEditor(item, currentType) {
		...
		const fieldsHtml = typeConfig.fields
			.map(
				(field) => `
	...
	updateEditorFields(newType) {
		...
		const typeFieldsContainer = editor.querySelector(
			".source-editor__type-fields",
		);
		...
		typeFieldsContainer.innerHTML = typeConfig.fields
			.map(
				(field) => `
      <label class="source-editor__field">
        <span class="source-editor__label">${escapeHtml(field.label)}</span>
