...
function applyInlineFormatting(text) {
	...
	output = output.replace(
		/@@INLINE(\d+)@@/g,
		(_, index) => inlineBlocks[Number(index)] ?? "",
	);
	return output;
}
...
export function renderMarkdown(raw) {
	...
	const splitTableRow = (row) => {
		...
		const withoutEdges = trimmedRow.startsWith("|")
			? trimmedRow.slice(1)
			: trimmedRow;
		const normalized = withoutEdges.endsWith("|")
	...
	const renderColumns = (columns) => {
		const columnHtml = columns
			.map(
				(column) =>
					`<div class="md-column">${applyInlineFormatting(column)}</div>`,
			)
			.join("");
	...
	for (let index = 0; index < lines.length; index += 1) {
		...
		if (isTitledQuoteBox) {
			...
			const looksLikeTitle =
				trimmed.includes(":") || /[.!?]$/.test(trimmed) || trimmed.length < 100;
			...
			if (looksLikeTitle) {
				...
				const quoteContent = quoteLines
					.map((value) => applyInlineFormatting(value))
					.join("<br />");
